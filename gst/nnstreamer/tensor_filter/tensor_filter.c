...
gst_tensor_filter_start (GstBaseTransform * trans)
{
  ...
  /**
   * @todo invoke-async mode
   * If invoke-async is enabled, sub-plugin allocates new memory and tensor-filter generates gst-buffer from the results.
   * 1. Define new event (SET_CALLBACK) and structure for tensor-filter callbacks.
   * 2. Implement new function to push gst-buffer for tensor-filter (or new-data event from API).
   * 3. Call new event SET_CALLBACK after opening the fw, so that sub-plugin can push invoke results.
   */
