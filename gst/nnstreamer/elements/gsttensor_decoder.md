...
tensor_decoder gives each set of `mode` or `optionN`, and each change of the incoming tensor config, new private data: it calls `init ()` and then `setOption ()` with the stored options, in the order they were set since the last `mode` set or tensor config change, which themselves give them in numbered order. If the stream is already negotiated when `mode` or `optionN` is set, the new private data then gets `getOutCaps ()` with the negotiated config before it is used, so what a sub-plugin sets up there is ready for `decode ()`, and tensor_decoder renegotiates its output caps. The previous private data is released with `exit ()` once no `getOutCaps ()`, `getTransformSize ()` or `decode ()` call is using it any more, and those calls are made without any lock of tensor_decoder held. So a sub-plugin does not see `setOption ()` on private data that is in use. Any state it builds up in `decode ()` across buffers (e.g., the object tracking of `bounding_boxes`, or the state of a python3 decoder script) starts over when an option is set during streaming, and a sub-plugin that refuses the negotiated config after the change stops the stream as a failed negotiation.
