...
class TensorFilterLlamaCpp : public tensor_filter_subplugin
{
  ...
  std::queue<std::tuple<GstTensorFilterProperties *, std::string, GstTensorMemory *>> input_queue;
...
TensorFilterLlamaCpp::outputThreadLoop ()
{
  while (!stop_thread) {
    ...
    if (stop_thread && input_queue.empty ())
      break;

    auto [prop, prompt, output] = std::move (input_queue.front ());
    input_queue.pop ();
    lock.unlock ();
    try {
      this->generateTokens (prop, std::move (prompt), output);
