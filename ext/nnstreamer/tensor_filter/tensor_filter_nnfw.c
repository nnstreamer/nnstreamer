...
#define NNFW_DEFAULT_BACKEND NNFW_NEON_BACKEND
#else
...
nnfw_open (const GstTensorFilterProperties * prop, void **private_data)
{
  ...
  if (*private_data != NULL) {
    pdata = *private_data;
    if (g_strcmp0 (prop->model_file, pdata->model_file) != 0) {
  ...
  model_path = g_path_get_dirname (prop->model_file);
  ...
  if (nnfw_metadata_verify (meta_file, prop->model_file) != 0) {
    err = -EINVAL;
  ...
  pdata->model_file = g_strdup (prop->model_file);
  g_free (meta_file);
  g_free (model_path);
