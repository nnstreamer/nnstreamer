...
class executorch_llama_subplugin final : public tensor_filter_subplugin
{
  ...
  std::string model_path; /**< The model *.pte file */
  std::string tokenizer_path; /**< The tokenizer file */
...
executorch_llama_subplugin::configure_instance (const GstTensorFilterProperties *prop)
{
  try {
    ...
    if (!g_file_test (prop->model_files[1], G_FILE_TEST_IS_REGULAR)) {
      const std::string err_msg
          = "Given file " + (std::string) prop->model_files[1] + " is not valid";
      throw std::invalid_argument (err_msg);
    }

    model_path = prop->model_files[0];
    tokenizer_path = prop->model_files[1];

    /* set llama runner */
    runner = std::make_unique<example::Runner> (model_path, tokenizer_path, 0.8f);
