...
# CI copies the prebuilt ggml backends of llama.cpp into build/tests, so the
# directory alone would make them ours.
run_case object_ggml_n <<'EOF'
==1== Command: ./tests/unittest_filter_llamacpp
==1== Conditional jump or move depends on uninitialised value(s)
==1==    at 0x7097693: ggml_graph_compute (in /home/runner/work/nnstreamer/nnstreamer/build/tests/libggml-cpu-haswell.so)
==1==
EOF
expect_status 0 "an error in a ggml backend copied into the build directory is not ours"
expect_in "1 from libraries" "that error is counted as a library one"
...
==1== Command: ./tests/unittest_filter_llamacpp
==1== 416 bytes in 1 blocks are definitely lost in loss record 7 of 9
==1==    at 0x1111: calloc (vg_replace_malloc.c:1328)
==1==    by 0x2222: ggml_graph_compute (in /home/runner/work/nnstreamer/nnstreamer/build/tests/libggml-cpu-haswell.so)
==1==    by 0x3333: start_thread (pthread_create.c:442)
==1==
EOF
...
==1==    at 0x1111: calloc (vg_replace_malloc.c:1328)
==1==    by 0x2222: ggml_graph_compute (in /home/runner/work/nnstreamer/nnstreamer/build/tests/libggml-cpu-haswell.so)
==1==    by 0x3333: my_test_body (myelement.c:50)
==1==
EOF
...
# The runner reaches its suppression file through a relative path. When that
# file cannot be opened, valgrind stops the same way after its banner, trimmed.
run_case suppressions_missing_required_n --require-output <<'EOF'
==10437== Memcheck, a memory error detector
==10437== Command: ./tests/unittest_demo
==10437==
--10437-- Valgrind options:
--10437--    -v
--10437--    --suppressions=../tools/debugging/valgrind_suppression
--10437-- Reading suppressions file: ../tools/debugging/valgrind_suppression
==10437== FATAL: can't open suppressions file "../tools/debugging/valgrind_suppression"
EOF
expect_status 1 "--require-output rejects a run whose suppression file could not be opened"
expect_in "[unittest_demo] no ERROR SUMMARY" "and names the binary that was never checked"
...
# Process ids wrap around. A later process that happens to get the id of a run
# that stopped short must not finish that run with its own summary.
run_case reused_pid_n --require-output <<'EOF'
==5== Command: ./tests/unittest_one
==6== Command: ./tests/unittest_two
==5== ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0)
==6== ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0)
EOF
expect_status 1 "a reused process id does not finish an earlier run that stopped short"
expect_in "[unittest_one] no ERROR SUMMARY" "the run that stopped short is named"
expect_not_in "[unittest_two] no ERROR SUMMARY" "the run that finished is not"

# The timestamps differ, as they do on real lines, so the process id has to be
# taken after the timestamp is stripped.
run_case timestamped_required --require-output <<'EOF'
2026-09-07T07:21:27.4959062Z ==1== Command: ./tests/unittest_demo
2026-09-07T07:21:31.0271853Z ==1== ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0)
EOF
