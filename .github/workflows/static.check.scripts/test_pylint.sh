...
# fail the checker, also in a non-ASCII file, and convention, refactor,
# warning and import-error messages must not. The first would have caught the
# checker silently passing on every input (#4919); the second keeps it from
# rejecting nearly every Python file in the tree, or every file that imports
# a module CI does not install. The gi case only bites where python3-gi is
# installed without the GStreamer typelib, as in Ubuntu's server image.
...
cat > "${workdir}/gi_gst.py" << 'EOF'
"""A script importing GStreamer through gi, as the tensor_region tests do."""
import gi
...
cat > "${workdir}/os_missing_name.py" << 'EOF'
"""A script importing a name that its module does not have."""
from os import no_such_name

print(no_such_name)
EOF

# A non-ASCII comment makes file(1) report UTF-8 text instead of ASCII text.
printf '"""A script with a non-ASCII comment."""\n# caf\303\251\nprint(undefined_name)\n' \
  > "${workdir}/utf8_error.py"
if file "${workdir}/utf8_error.py" | grep -q "ASCII"; then
  echo "FAIL: utf8_error.py is not classified as non-ASCII text"
  failed=1
fi
...
expect_checker 1 "error in a file with a non-ASCII comment" "${workdir}/utf8_error.py"
expect_log "utf8_error\.py:[0-9]+:[0-9]+: E0602:" "the non-ASCII file is linted"
expect_checker 1 "missing name imported from a module other than gi.repository" "${workdir}/os_missing_name.py"
expect_log "os_missing_name\.py:[0-9]+:[0-9]+: E0611:" "only gi.repository is exempt from E0611"
...
expect_checker 0 "gi.repository import without the GStreamer typelib" "${workdir}/gi_gst.py"
expect_log "Checking .*/gi_gst\.py" "the gi file is checked"
