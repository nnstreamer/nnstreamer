...
/**
 * @brief Test fixture class for tensor_filter SFlare plugin unit tests.
 */
class NNStreamerFilterSFlareCppTest : public ::testing::Test
{
  protected:
  ...
  public:
  /**
   * @brief Constructor a new NNStreamerFilterSFlareCppTest object
   */
  NNStreamerFilterSFlareCppTest ()
  {
    loaded = FALSE;
    skip_test = FALSE;
  }
...
#if 0
    data_file = g_build_filename (root_path, "tests", "test_models", "data", "sflare_input.txt", NULL);
    tokenizer = g_build_filename (root_path, "tests", "test_models", "data", "tokenizer.json", NULL);
#else
    data_file = g_strdup ("./sflare_input.txt");
    tokenizer = g_strdup ("./tokenizer.json");
#endif
    loaded = TRUE;
  }
  ...
  void create_pipeline (const gchar *model, gboolean invoke_async, const gchar * custom)
  {
    pipeline_str = g_strdup_printf (
        ...
        "tensor_filter name=tensor_filter framework=sflare model=%s invoke-dynamic=TRUE invoke-async=%d custom=%s ! "
 ...
 */
 ...
 /**
 * @brief Test case for tensor_filter SFlare plugin with singleInputSingleOutputSync_p
 */
TEST_F (NNStreamerFilterSFlareCppTest, singleInputSingleOutputSync_p)
{
  gboolean invoke_async = FALSE;
  gchar *contents = NULL;
  gsize length = 0;
  const g_autofree gchar *custom = g_strdup_printf ("tokenizer_path:%s,backend:CPU,output_size:1024,model_type:3B,data_type:W4A32", tokenizer);
...
/**
 * @brief Test case for tensor_filter SFlare plugin with multipleInputsSingleOutputSync_p
 */
TEST_F (NNStreamerFilterSFlareCppTest, multipleInputsSingleOutputSync_p)
{
  gboolean invoke_async = FALSE;
  gchar *contents = NULL;
  gsize length = 0;
...
#if 0 /*SFlare call exit(0)*/
/**
 * @brief Test case for tensor_filter SFlare plugin with invalidTokenizerPath_n
 */
TEST_F (NNStreamerFilterSFlareCppTest, invalidTokenizerPath_n)
{
  gboolean invoke_async = FALSE;
  gchar *contents = NULL;
  gsize length = 0;
  const g_autofree gchar *custom = g_strdup_printf ("tokenizer_path:invalidTokenizerPath,backend:CPU,output_size:1024,model_type:3B,data_type:W4A32");
...
#if 0 /* SFlare use hardcoded model path internally */
/**
 * @brief Test case for tensor_filter SFlare plugin with invalidModel_n
 */
TEST_F (NNStreamerFilterSFlareCppTest, invalidModel_n)
{
...
/**
 * @brief Test case for tensor_filter SFlare plugin with earlyTerminationBeforeTokenGenerationSync_n
 */
TEST_F (NNStreamerFilterSFlareCppTest, earlyTerminationBeforeTokenGenerationSync_n)
{
  gboolean invoke_async = FALSE;
  gchar *contents = NULL;
  gsize length = 0;
