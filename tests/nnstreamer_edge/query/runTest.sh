...
_callCompareTest raw1.1_1.log result1.1_1.log 1.1-4 "Compare 1.1-4" 1 0
# The third result should fail because third input buffer has been dropped.
_callCompareTest raw1.1_2.log result1.1_2.log 1.1-5 "[Fail test] Compare 1.1-5" 1 1
kill -9 $pid &> /dev/null
wait $pid
...
gstTest "--gst-plugin-path=${PATH_TO_PLUGIN} \
    ...
    tensor_converter ! other/tensors,format=flexible ! tee name=t \
        ...
        t. ! queue ! tensor_query_client dest-port=${PORT1} port=${PORT3} ! multifilesink location=result5_2_%1d.log" 5-2 0 0 $PERFORMANCE $TIMEOUT_SEC
...
gstTest "--gst-plugin-path=${PATH_TO_PLUGIN} \
    ...
    tensor_converter ! other/tensors,format=flexible ! tee name=t \
        ...
        t. ! queue ! tensor_query_client dest-port=${PORT2} port=${PORT4} ! multifilesink location=result5_3_%1d.log" 5-3 0 0 $PERFORMANCE $TIMEOUT_SEC
