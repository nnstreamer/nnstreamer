...
/**
 * @brief The element flags of tensor_src_grpc must not alias those of GstBaseSrc.
 */
TEST (nnstreamerGrpc, srcElementFlags)
{
  guint base = (guint) GST_BASE_SRC_FLAG_STARTING | (guint) GST_BASE_SRC_FLAG_STARTED;

  EXPECT_EQ ((guint) GST_TENSOR_SRC_GRPC_CONFIGURED & base, 0U);
  EXPECT_EQ ((guint) GST_TENSOR_SRC_GRPC_STARTED & base, 0U);
}
