...
PATH_TO_MODEL="../test_models/models/yolov5nu.onnx"
...
gstTest "--gst-plugin-path=${PATH_TO_PLUGIN} \
    ...
    imagefreeze ! \
    videoconvert ! \
    video/x-raw,width=320,height=320,format=RGB,framerate=0/1 ! \
    tensor_converter ! \
    tensor_transform mode=transpose option=1:2:0:3 ! \
    tensor_transform mode=arithmetic option=typecast:float32,div:255 ! \
    tensor_filter framework=tensorrt model=${PATH_TO_MODEL} ! \
    tensor_transform mode=transpose option=1:0:2:3 ! \
    tensor_decoder mode=bounding_boxes option1=yolov8 option2=${PATH_TO_LABEL} option3=1 option4=320:320 option5=320:320 ! \
