...
- [tensor\_split](https://github.com/nnstreamer/nnstreamer/tree/main/gst/nnstreamer/elements/gsttensor_split.md) (stable)
  ...
  - A source pad keeps the caps it was created with. Changing ```tensorseg```, ```tensorpick```, or the input type or framerate while the pipeline runs does not update them; stop the pipeline to ```READY``` or ```NULL``` to apply such a change.
