...
- [tensor\_split](https://github.com/nnstreamer/nnstreamer/tree/main/gst/nnstreamer/elements/gsttensor_split.md) (stable)
  ...
  - Users may output only some of the segments with ```tensorpick```. The picked segments go out on ```src_0```, ```src_1```, ... in ascending segment order, and each pad describes the segment it carries: with ```tensorseg=1:4:4,2:4:4 tensorpick=1```, ```src_0``` carries ```dimensions=2:4:4```.
